...
import sd1920.trab1.clients.utils.InsecureHostnameVerifier;
...
import sd1920.trab1.clients.utils.SendMessInfo;
...
public class MessageResourceRest implements MessageService {
	...
	public MessageResourceRest() {
		MessageResourceRest.randomNumberGenerator = new Random(System.currentTimeMillis());
		MessageResourceRest.allMessages = new HashMap<Long, Message>();
		MessageResourceRest.userInboxs = new HashMap<String, Set<Long>>();
		this.information = new HashMap<String, BlockingQueue<SendMessInfo>>();
		this.threads = new LinkedList<Thread>();
	}

	public MessageResourceRest(Discovery dis, int port) {
		MessageResourceRest.randomNumberGenerator = new Random(System.currentTimeMillis());
		this.dis = dis;
		MessageResourceRest.allMessages = new HashMap<Long, Message>();
		MessageResourceRest.userInboxs = new HashMap<String, Set<Long>>();
					...
					if (serverUrl.contains("rest")) {
						...
						try {
							if (toSend.toPostMessage()) {
								...
								Response r = target.path(("" + m.getId())).request().delete();
						...
						if (toSend.toPostMessage()) {
							...
							try {
								received = messages.receiveMessageFromOut(m);
							...
							try {
								messages.deleteMessageFromOut(("" + m.getId()));
								toSend = null;
							} catch (WebServiceException wse) {
								wse.printStackTrace();
							}
	...
	public void deleteMessage(String user, long mid, String pwd) {
		synchronized (this) {
			if (!userInboxs.containsKey(user) || getUser(user, pwd) == null) {
				Log.info("The sender does not exist or if the pwd is not correct");
				throw new WebApplicationException(Status.FORBIDDEN);
			}
			...
			if (allMessages.containsKey(mid)) {
				...
				MessageUtills.printMessage(allMessages.get(mid));
	...
	@Override
	public void deleteMessageFromOut(String id) {
	...
	@Override
	public String[] receiveMessageFromOut(Message message) {
	...
	@Override
	public void deleteInbox(String user) {
		synchronized (this) {
	...
	/**
	 * 
	 * Auxiliary method to get a user with his name and password
	 * 
	 * @param name name of the user to get
	 * @param pwd  password of the user
	 * @return the User with the respective name if the password is correct, returns
	 *         null otherwise
	 */
	private User getUser(String name, String pwd) {
		...
		config.property(ClientProperties.READ_TIMEOUT, REPLY_TIMEOUT);
		Client client = ClientBuilder.newClient(config);
		WebTarget target = client.target(getLocalServerURI()).path(UserService.PATH);

		if (pwd != null)
			target = target.queryParam("pwd", pwd);

		Response r = target.path(name).request().accept(MediaType.APPLICATION_JSON).get();

		User u = null;
		if (r.getStatus() == Status.OK.getStatusCode() && r.hasEntity()) {
			Log.info("Success:");
			u = r.readEntity(User.class);
			UserUtills.printMessage(u);
		} else
			Log.info("Error, HTTP error status: " + r.getStatus());
